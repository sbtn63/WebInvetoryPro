...
class UserChangeInfoForm(forms.ModelForm):
    ...
    current_password = forms.CharField(widget=forms.PasswordInput(), min_length=8)
    ...
    def clean_username(self):
        ...
        if username and username != self.current_user.username:
            if (
                User.objects.exclude(pk=self.current_user.pk)
    ...
    def clean(self):
        ...
        if password_new or password_new_confirm:
            if not password_new:
                self.add_error(
                    "password_new", "Debes proporcionar la nueva contraseña."
                )
            if not password_new_confirm:
                self.add_error(
                    "password_new_confirm", "Debes confirmar la nueva contraseña."
                )
            if (
                password_new
                and password_new_confirm
                and password_new != password_new_confirm
            ):
